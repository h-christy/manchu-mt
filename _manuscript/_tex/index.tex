...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, bottomtitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{The Language Manchu}, titlerule=0mm, colbacktitle=quarto-callout-note-color!10!white, breakable, colframe=quarto-callout-note-color-frame, rightrule=.15mm, toptitle=1mm, toprule=.15mm, bottomrule=.15mm, leftrule=.75mm, left=2mm, opacitybacktitle=0.6, colback=white, opacityback=0]
...
\item
  Digitization challenges: As some may have observed that the Manchu
  writing system is very far from that of English or the most common
  writing systems in the world. First, the direction, in
  \textbf{columns} and from left to right, similar to Arabic rotated 90
  degrees counter-clockwise. Second, Manchu script (alphabets) can be
  described as \textbf{IMFI} script which letters can take on more than
  four different shapes depending on where they appear in the word or
  whether they are in isolation. This complicated the picture for OCR
  digitization work, due to the extra work on letter segmentation.
\item
  Transliteration
\item
  Translation
...
Manchu-Korean case, but more generally. They thus come up with an
...
this way, they witness a significant improvement in the BLEU score of
their seq2seq model, from 0 to 38, as the number of sentence pairs grows
from 20K to 179K.
...
This study, from the same SNU team, continues their line of work and
with their data augmentation technique, they further contribute by
training the Manchu Langauge Model \textbf{manchuBERT} from scratch and
releasing the model on Hugging Face. The manchuBERT langauge model
serves as the starting point for developing NER and POS tagging for
Manchu. The strong performance of these downstream NLP models, in turn,
provides evidence that manchuBERT langauge model has indeed learned to
be a robust encoder for Manchu.
...
of linguistic information. The conclusion they arrive at is that--
...
hope that translators of this canonical work would place stronger
emphasis on fidelity to the original text, thus, expecting it to be
relatively easier for me to find sentence-level alignments that would
help me a lot in building my language pairs
...
I reserved 12.5\% for validation (37 rows), as for the another 38 rows
for testing, leaving 225 rows for training. Given the very limited size
of our data, and the often large number of training parameters in our
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, bottomtitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{The issue of Transliteration Alignment}, titlerule=0mm, colbacktitle=quarto-callout-note-color!10!white, breakable, colframe=quarto-callout-note-color-frame, rightrule=.15mm, toptitle=1mm, toprule=.15mm, bottomrule=.15mm, leftrule=.75mm, left=2mm, opacitybacktitle=0.6, colback=white, opacityback=0]
...
initially for my self-curated dataset was the
\textbf{Möllendorff-Norman} system, because it is the one our Manchu
professor taught, and is also one of the most used systems.
...
to find out that they do not fully adopt the system. To clarify this,
and because the datasets disclosed in their github repository are the
test sets, I ran a small experiment using manchuBERT as a masked
language model. After inputting some sentences with mask tokens, the
outputs of manchuBERT suggested that they actually use a hybrid system
for transliteration. For example, the model represents the sixth Manchu
vowel using \texttt{v} rather than the Norman \texttt{ū}; uses
\texttt{x} instead of the Norman \texttt{š}, which seems to be aligning
to the Abkai system. At the same time, it employs Norman \texttt{c}
rather than the Abkai-preferred \texttt{q}.
...
attention in the data preprocessing stage.
...
(\citeproc{ref-DBLP:journalsux2fcorrux2fabs-1810-04805}{2018})).\footnote{with
  manually handling and}In \emph{Attempt 3}, I continue to use the
manchuBERT tokenizer for Manchu source sentences; the English
counterpart uses the SentencePiece tokenization, which does the
\textbf{subword} segmentation, and I have to additionally specify the
tokenizer to translate into English. Finally, \emph{Attempt 4}, the
sentence was feed to google colab AI directly.
