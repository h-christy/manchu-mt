...
This project is an attempt to build a Neural Machine Translation model
for the challenging task of Manchu-English translation.
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{The Language Manchu}, opacityback=0, arc=.35mm, titlerule=0mm, breakable, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, colback=white, left=2mm, leftrule=.75mm, bottomtitle=1mm, rightrule=.15mm, bottomrule=.15mm]

\pandocbounded{\includegraphics[keepaspectratio]{images/manchu-steps.png}}

\subparagraph{Our Goal and the Challenges Along the
...
\tightlist
\item
  Language in Danger: Manchu is often treated like Latin, mainly learned
  to read historical documents. Unlike Latin, however, it still has a
  spoken tradition today. In the \textbf{Qapqal Xibe Autonomous County}
  in Xinjiang, where its use is part of deliberate preservation efforts,
  taken by the Xibe people in the name of the Xibe (Sibe) language
\item
  Digitization challenges: As some may have observed, the Manchu writing
  system is a distinct one, comparing to some common writing systems in
  the world. First, the direction, in \textbf{columns} and from left to
  right, similar to Arabic rotated 90 degrees counter-clockwise. Second,
  Manchu script (alphabets) can be described as \textbf{IMFI} script
  which letters can take on more than four different shapes depending on
  where they appear in the word or whether they are in isolation. This
  complicates the picture for digitization work using Optical character
  recognition (OCR), due to the extra work on letter segmentation.
\item
  Transliteration
\item
  Translation
...
introduced me to the task of Manchu machine translation and helped shape
the methods and approaches in this project. These studies, in their
distinctive ways, engage the Manchu Language revitalization effort
within the broader dialogue of Neural Machine Translation in
...
This paper introduces us to the challenges of developing the typically
data-driven neural machine translation models for Manchu, which suffers
from (an extreme) scarcity of parallel data, not only in the
Manchu-Korean case, but more generally. The authors thus come up with an
innovative data augmentation strategy based on \textbf{word
replacement}, where lexical similarity is determined using GloVe (Global
Vectors for Word Representation) embeddings of dictionary items. By
augmenting the parallel corpus in this way, they witness a significant
improvement in the BLEU (Bilingual Evaluation Understudy) score of their
seq2seq (Sequence-to-Sequence) model, from 0 to 38, as the number of
sentence pairs grew from 20K to 179K.
...
This study, from the same Seoul National University (SNU) team,
continues their line of work and with their data augmentation technique,
they further contribute by training the Manchu Langauge Model
\textbf{manchuBERT} from scratch and releasing the model on Hugging
Face. The manchuBERT langauge model serves as the starting point for
developing NER (name entity recognition) and POS (part-of-speech)
tagging for Manchu. The strong performance of these downstream NLP
models, in turn, provides evidence that manchuBERT langauge model has
indeed learned to be a robust encoder for Manchu.
...
This third paper also discusses the prospects of low-resource machine
translation, but approaches the problem from a different angle. Rather
than focusing on model architectural modifications or data augmentation
techniques, the authors consider how \textbf{human-curated linguistic
knowledge}, such as dictionaries and grammar books can contribute in
this picture. Specifically, they explore how linguistic resources can be
instilled into pre-trained LLMs as such models can leverage their
in-context learning (\textbf{ICL}) capability for the translation of
languages they were not trained on. Through a series of designed
experiments, they systematically examine the effects of different types
of linguistic information. The authors arrived at the conclusion that
dictionaries that comprehensively cover lexical meanings, suffix
explanations, and morpheme collocation information can effectively
improve translation quality, as can parallel examples.

\subsection{Data}\label{data}

\subsubsection{Data Collection}\label{data-collection}

The data used in this project is a small but carefully examined
Manchu-English parallel dataset. I chose the renowned novel
\textbf{\emph{The Dream of the Red Chamber}} written in classical
Chinese by Cao Xueqin (published in 1790s Qing China)\footnote{the novel
  was written in ``Written Vernacular Chinese'', but still not that as
  explicit as modern Chinese}, in specific, the novel's Manchu
translation(曹雪芹
(\citeproc{ref-CaoXueQinHongLouMengXiBoWen1993}{1993})) and English
translation (Cao (\citeproc{ref-caoHungLouMeng2006}{2006})) I am hoping
that translators of this canonical work would place stronger emphasis on
fidelity to the original text, thus, expecting it to be relatively
easier for me to find sentence-level alignments that would support my
endeavor to build Manchu-English sentence pairs
...
\pandocbounded{\includegraphics[keepaspectratio]{images/sent-plot.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/mnc-word-freq-plot.png}}

\pandocbounded{\includegraphics[keepaspectratio]{images/en-word-freq-plot.png}}

\paragraph{Data Split}\label{data-split}

I reserved 12.5\% for validation (37 rows), and 38 rows for testing,
leaving 225 rows for training. Given the very limited size of our data,
and the often large number of training parameters in our neural models,
regularization and monitoring validation loss are both very crucial
concerns in the training process.\footnote{a better and definitely
  doable robustness test is to implement k-fold cross validation so as
  to eliminate the effect of data splitting for small sample}
...
For simplicity, I removed all punctuation, and all sentences were
converted to lowercase.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{The issue of Transliteration Alignment}, opacityback=0, arc=.35mm, titlerule=0mm, breakable, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, colback=white, left=2mm, leftrule=.75mm, bottomtitle=1mm, rightrule=.15mm, bottomrule=.15mm]

I faced challenges in the data preparation phase is around the issue of
transliteration, and I found it worth recording, so I would like to
share it here. The transliteration system that I was using initially for
my self-curated dataset was the \textbf{Möllendorff-Norman} system,
because it is the one our Manchu professor taught, and is also one of
...
\textbf{Abkai transliteration}. I converted my dataset into Abkai, only
to find out that the authors do not fully adopt this transliteration
system. To clarify this, and because the datasets disclosed in their
github repository are the test sets, I ran a small experiment using
manchuBERT as a masked language model. After inputting some sentences
with mask tokens, the outputs of manchuBERT suggested that they actually
use a hybrid system for transliteration. For example, the model
represents the sixth Manchu vowel using \texttt{v} rather than the
Norman \texttt{ū}; uses \texttt{x} instead of the Norman \texttt{š},
which seems to be aligning to the Abkai system. At the same time, it
employs Norman \texttt{c} rather than the Abkai-preferred \texttt{q}.

Based on these observations, I re-romanized the Manchu side of my data
to align with what manchuBERT learnt. I think this transliteration
alignment process that I underwent actually highlights a non-trivial
challenge in obtaining and integrating online Manchu resources: when
without a standardized transliteration system, languages that use
non-Latin scripts are going to suffer, or will require additional
attention in the data pre-processing stage.

\end{tcolorbox}

\paragraph{Tokenization}\label{tokenization}

Different ways for tokenizations are implemented across my four MT
...
whitespace, resulting in word-level tokenization.\footnote{with four
  additional tokens, , , ,} In \emph{Attempt 2}, to correctly map to
...
tokenizer was applied (Devlin et al.
(\citeproc{ref-DBLP:journalsux2fcorrux2fabs-1810-04805}{2018})).\footnote{with
  manually handling and}In \emph{Attempt 3}, I continued to use the
manchuBERT tokenizer for Manchu source sentences; the English
counterpart used the SentencePiece tokenization, which completes the
\textbf{subword} segmentation, and I had to additionally specify the
tokenizer to translate into English. Finally, in \emph{Attempt 4}, I
carried out a similar practice as of the Pei et al.~(Pei, Liu, Lin,
Yvon, and Schuetze (\citeproc{ref-pei-etal-2025-understanding}{2025})),
I wrapped up the sentences with corresponding prompts and fed directly
to google colab AI.
...
This is the baseline model I used, and I directly referred to the
...
\item
  ...
  which I used the \texttt{Automodel} class from transformers library to
  load the \textbf{manchuBERT} model configuration from Hugging Face,
  and then I used the method \texttt{from\_pretrained()} to acquire the
  ...
  sentences\footnote{195,611 monolingual Manchu sentences before data
    augmentation} The embedding vector is of 768 length. With the help
  of manchuBERT our raw Manchu input can be transformed into rich and
  context-aware representation space, as an encoder I pair it with an
  LSTM decoder for our downstream translation task.
\end{itemize}
...
\item
  ...
  encodings are passed to the \textbf{mBART},\footnote{One of the key
    contributions of the mBART model is that it is proven to be
    conducive to translation of even the languages unseen by the model
    to fluent English sentences} the pre-trained, \textbf{end-to-end}
  multilingual to English model there to support the next step that
  takes encoded Manchu sentences to sensical English.(Liu et al.
  (\citeproc{ref-liuMultilingualDenoisingPretraining2020}{2020}))\footnote{https://huggingface.co/transformers/v4.4.2/\_modules/transformers/models/mbart/tokenization\_mbart50\_fast.html}
\end{itemize}
...
\item
  ...
  I explored the linguistic resource \emph{The Comprehensive
  Manchu-English Dictionary} from Jerry Norman(Norman
  (\citeproc{ref-normanComprehensiveManchuEnglishDictionary2013}{2013}))
  Step by step, I first parse the dictionary into
  \texttt{item}-\texttt{meaning} pairs of 21K, and I retrieve the
  vocabularies from our source Manchu sentence and incorporate the
  dictionary information as part of the translation prompt
\item
  One more special account for Attempt 4 is that rather than directly
  enciphering our source to eliminate the prior knowledge of our LLM, I
  introduced some controls to the experiment by speicifying that the
  task is Manchu-to-English or not specifying the target language, or by
  creating a substitute such as ``Anchum'' language in the prompt.
\end{itemize}
...
\item
  ...
  precision} plus a \textbf{brevity penalty}, as it is intuitive, for
  that a translation task does not have a single ground truth or
  absolute correctness. Thus, in the evaluation of machine translation
  models, test cBLEU is the most used metric.
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{images/att1-loss-plot.png}}

Plot 2: Attempt 2 Training Trace

\pandocbounded{\includegraphics[keepaspectratio]{images/att2-loss-plot.png}}

Plot 3: Attempt 3 Training Trace

\pandocbounded{\includegraphics[keepaspectratio]{images/att3-loss-plot.png}}

Across all three NMT models, the training loss decreased monotonically,
indicating these algorithms' ability to fit our training data. In
contrast, as for the validation loss curves, three models all achieved
the lowest validation loss before epoch 5, which reflects the contraints
...
best model based on validation loss, the manchuBERT + LSTM model
...
other two. This aligns with our understanding of the relationship
between data size and the size of training parameters.

I observed that in theory, the combined manchuBERT + mBART model
arrangement made sense. However, in implementation, I wrestled with the
tokenization alignment quite a bit only to get it working and the
training was very slow. This to a certain extent points me to an
understanding of model flexibility, that the model architecture of my
third attempt was a bit too grand and sophisticated with respect the 300
language pairs that I had at hand.

Moving towards my fourth attempt, I was interested in examining,
provided linguistic knowlege and lexical meanings of the words in a
sentence alone, LLM's capability to do translation on languages that
they were not explicitly trained on. Thus, I design an experiment with
hope that I can to some extent eliminate the prior knowledge of Manchu
of LLM. Specifically, I referred to the Pei et al.~paper(Pei, Liu, Lin,
Yvon, and Schütze
(\citeproc{ref-pei2025understandingincontextmachinetranslation}{2025})),
and performed a comparison between four conditions created by different
...
While the performance drops when there is misinformation or no
information about the source language. However, from the generated
translations, we can see that LLMs are good at forming fluent English
...
or suffixations in Manchu grammar, one concrete example is that when a
word in a Manchu sentence does not have a dictionary entry, the absence
of a English definition could easily causes the translation quality to
drop.
...
decent translations, nevetheless this approach is vulnerable when words
in a sentence did not find a mapping in dictionary, such as in the case
of ID 4, ``loo tai tai''---a Manchu transliteration of a Chinese word
meaning old lady, does not appear in the dictionary, but they are
mismatched to `LOO: prison; gong; cf.~lo; see lomi', `TAI: platform,
terrace, stage', which drives the translation off the correct path.
Interestingly, for the same ID 4 sentence translation, Attempt
3(manchuBERT + mBART) somewhat captured the meaning of loo tai tai in
the sentence. The cases that the translation of Attempt 3 are
underestimated, ID 16 is one example and it almost captures the first
half of the sentence, but later falls into decoder degeneration.
...
which aligns with part of the sentence pretty well.
...
obstacles of data scarcity in a NMT model, is meaningful to me because I
finally get to go from not just vaguely think about how I can use data
...
\bibitem[\citeproctext]{ref-DBLP:journalsux2fcorrux2fabs-1810-04805}
