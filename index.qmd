...
This paper introduces us the challenges of developing the typically data-driven neural machine translation models for Manchu, which suffers from (an extreme) scarcity of parallel data, not only in the Manchu-Korean case, but more generally. They thus come up with an innovative data augmentation strategy based on **word replacement**, where lexical similarity is determined using GloVe embeddings of dictionary items. By augmenting the parallel corpus in this way, they witness a significant improvement in the BLEU score of their seq2seq model, from 0 to 38, as the number of sentence pairs grows from 20K to 179K.

##### Lee et al., *“ManNER & ManPOS: Pioneering NLP for Endangered Manchu Language.”*^[@leeManNERManPOSPioneering2024]

This study, from the same SNU team, continues their line of work and with their data augmentation technique, they further contribute by training the Manchu Langauge Model **manchuBERT** from scratch and releasing the model on Hugging Face. The manchuBERT langauge model serves as the starting point for developing NER and POS tagging for Manchu. The strong performance of these downstream NLP models, in turn, provides evidence that manchuBERT langauge model has indeed learned to be a robust encoder for Manchu.
...
This third paper also discusses the prospects of low-resource machine translation, but approaches the problem from a different angle. Rather than focusing on model architectural modifications or data augmentation techniques, the authors consider how **human-curated linguistic knowledge**, such as dictionaries and grammar books can contribute in this picture. Specifically, they explore how linguistic resources can be instilled into pre-trained LLMs as such models can leverage their in-context learning (**ICL**) capability for the translation of languages they were not trained on. Through a series of designed experiments, they systematically examine the effects of different types of linguistic information. The conclusion they arrive at is that-- dictionaries that comprehensively cover lexical meanings, suffix explanations, and morpheme collocation information can effectively improve translation quality, as can parallel examples.
...
The data used in this project is a small but carefully examined Manchu-English parallel dataset. I chose the renowned novel ***The Dream of the Red Chamber*** written in classical Chinese by Cao Xueqin (published in 1790s Qing China)^[the novel was written in "Written Vernacular Chinese", but still not that as explicit as modern Chinese], in specific, the novel's Manchu translation(@CaoXueQinHongLouMengXiBoWen1993) and English translation (@caoHungLouMeng2006) With the hope that translators of this canonical work would place stronger emphasis on fidelity to the original text, thus, expecting it to be relatively easier for me to find sentence-level alignments that would help me a lot in building my language pairs
...
I reserved 12.5% for validation (37 rows), as for the another 38 rows for testing, leaving 225 rows for training. Given the very limited size of our data, and the often large number of training parameters in our neural models, regularization and monitoring validation loss are both very crucial concerns in the training process.^[a better and definitely doable robustness test is to implement k-fold cross validation so as to eliminate the effect of data splitting for small sample]
